...
my $qq_substr  = qr{(?: KOI8U::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $function_getc;        # getc    to getc    or KOI8U::getc
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'KOI8U::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'KOI8U::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'KOI8U::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'KOI8U::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $KOI8U::re_t=%s; while($KOI8U::re_t =~ %s){%s local $^W=0; local $KOI8U::re_r=%s; %s$KOI8U::re_t="$`$KOI8U::re_r$'"; pos($KOI8U::re_t)=length "$`$KOI8U::re_r"; } return $KOI8U::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$KOI8U::re_r=eval $KOI8U::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $KOI8U::re_r=%s; %s"%s$KOI8U::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $KOI8U::re_n=0; while(%s =~ %s){%s local $^W=0; local $KOI8U::re_r=%s; %s%s="$`$KOI8U::re_r$'"; pos(%s)=length "$`$KOI8U::re_r"; $KOI8U::re_n++} return %s$KOI8U::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $KOI8U::re_r=%s; %s%s="%s$KOI8U::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $KOI8U::re_r=%s; %s%s="%s$KOI8U::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$KOI8U::re_r=eval $KOI8U::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
expand wildcard arguments supplied to programs. Instead, import of Ekoi8u.pm
works well.
