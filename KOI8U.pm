...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Ekoi8u::uc($char[$i]) ne Ekoi8u::lc($char[$i]))) {
            $char[$i] = '[' . Ekoi8u::uc($char[$i]) . Ekoi8u::lc($char[$i]) . ']';
        }
...
=item * Only KOI8U.pm and Ekoi8u.pm, other modules are unnecessary
...
functions.
  ...
  ---------------------------------
  lc          Ekoi8u::lc
  lcfirst     Ekoi8u::lcfirst
  uc          Ekoi8u::uc
  ucfirst     Ekoi8u::ucfirst
  chr         Ekoi8u::chr
  glob        Ekoi8u::glob
  ---------------------------------
...
KOI8U.pm remove 'bytes::' at head of function name.
...
KOI8U.pm remove '\' at head of alphanumeric regexp metasymbols \N, \p, \P
