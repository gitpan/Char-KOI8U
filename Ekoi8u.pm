...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Ekoi8u::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Ekoi8u::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Ekoi8u::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Ekoi8u::uc($_[0]) =~ m{\A $pattern \z}xms };
...
This module is a run-time routines of the KOI8U.pm.
Because the KOI8U.pm automatically uses this module, you need not use directly.
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Ekoi8u::split returns
...
=item Chop string
  ...
  The Ekoi8u::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Ekoi8u::index($string,$substr,$position);
  $pos = Ekoi8u::index($string,$substr);
  ...
  the Ekoi8u::index function returns -1.

=item Reverse index string

  $pos = Ekoi8u::rindex($string,$substr,$position);
  $pos = Ekoi8u::rindex($string,$substr);

  Works just like Ekoi8u::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Ekoi8u::chr(65) is "A" in either ASCII or KOI8-U, not Unicode,
  and Ekoi8u::chr(0x82a0) is a KOI8-U HIRAGANA LETTER A. For the reverse of
  Ekoi8u::chr, use KOI8U::ord.
