...
######################################################################
#
# Char::Ekoi8u - Run-time routines for Char/KOI8U.pm
#
...
BEGIN {
    ...
    sub qualify ($;$) {
        my ($name) = @_;
        if (!ref($name) && (Char::Ekoi8u::index($name, '::') == -1) && (Char::Ekoi8u::index($name, "'") == -1)) {
            my $pkg;
...
sub import {
    ...
    if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
        ...
        for (@ARGV) {
            ...
            # has space
            if (/\A (?:$q_char)*? [ ] /oxms) {
                if (my @glob = Char::Ekoi8u::glob(qq{"$_"})) {
                    push @argv, @glob;
                }
                else {
                    push @argv, $_;
...
#
sub unimport {}
sub Char::Ekoi8u::split(;$$$);
sub Char::Ekoi8u::tr($$$$;$);
sub Char::Ekoi8u::chop(@);
sub Char::Ekoi8u::index($$;$);
sub Char::Ekoi8u::rindex($$;$);
sub Char::Ekoi8u::lcfirst(@);
sub Char::Ekoi8u::lcfirst_();
sub Char::Ekoi8u::lc(@);
sub Char::Ekoi8u::lc_();
sub Char::Ekoi8u::ucfirst(@);
sub Char::Ekoi8u::ucfirst_();
sub Char::Ekoi8u::uc(@);
sub Char::Ekoi8u::uc_();
sub Char::Ekoi8u::fc(@);
sub Char::Ekoi8u::fc_();
sub Char::Ekoi8u::ignorecase;
sub Char::Ekoi8u::classic_character_class;
sub Char::Ekoi8u::capture;
sub Char::Ekoi8u::chr(;$);
sub Char::Ekoi8u::chr_();
sub Char::Ekoi8u::glob($);
sub Char::Ekoi8u::glob_();

sub Char::KOI8U::ord(;$);
sub Char::KOI8U::ord_();
sub Char::KOI8U::reverse(@);
sub Char::KOI8U::getc(;*@);
sub Char::KOI8U::length(;$);
sub Char::KOI8U::substr($$;$$);
sub Char::KOI8U::index($$;$);
sub Char::KOI8U::rindex($$;$);
sub Char::KOI8U::escape(;$);
...
${Char::Ekoi8u::not_space}   = qr{(?:[^\s\x0B])};
...
sub Char::Ekoi8u::lcfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Char::Ekoi8u::lc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Char::Ekoi8u::lc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Char::Ekoi8u::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}
...
sub Char::Ekoi8u::ucfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Char::Ekoi8u::uc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Char::Ekoi8u::uc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Char::Ekoi8u::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}
    ...
    for my $string (@string) {
        ...
        for (my $i=0; $i <= $#char; $i++) {
            ...
            # with /i modifier
                ...
                my $uc = Char::Ekoi8u::uc($char[$i]);
                my $fc = Char::Ekoi8u::fc($char[$i]);
                if ($uc ne $fc) {
                    if (CORE::length($fc) == 1) {
                        $char[$i] = '['   . $uc       . $fc . ']';
                    }
            ...
            for (@singleoctet) {
                while (s/ \A \\x(..) - \\x(..) //oxms or s/ \A \\x((..)) //oxms) {
                    for my $ord (hex($1) .. hex($2)) {
                        ...
                        my $uc = Char::Ekoi8u::uc($char);
                        my $fc = Char::Ekoi8u::fc($char);
                        if ($uc eq $fc) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $fc = Char::Ekoi8u::fc($char)) ne $char) {
                $pattern .= $fc;
            }
            else {
        ...
        my $matchsub = sub { Char::Ekoi8u::fc($_[0]) =~ /\A $pattern \z/xms };
...
INNER:
        for my $leaf (@leaf) {
            ...
            if (Char::Ekoi8u::index($leaf,'.') == -1 and   # if name does not have a dot in it *and*
                CORE::length($leaf) <= 8 and        # name is shorter than or equal to 8 chars *and*
                Char::Ekoi8u::index($pattern,'\\.') != -1  # pattern has a dot.
            ) {
                if (&$matchsub("$leaf.")) {
                    push @matched, "$head$leaf";
                    next INNER;
...
sub Char::KOI8U::getc(;*@) {
    ...
    croak 'Too many arguments for Char::KOI8U::getc' if @_ and not wantarray;
    ...
    for my $length ($length[0] .. $length[-1]) {
        ...
        if (exists $range_tr{CORE::length($getc)}) {
            if ($getc =~ /\A ${Char::Ekoi8u::dot_s} \z/oxms) {
...
sub Char::KOI8U::index($$;$) {
    ...
    my $index;
    if (@_ == 3) {
        $index = Char::Ekoi8u::index($_[0], $_[1], CORE::length(Char::KOI8U::substr($_[0], 0, $_[2])));
    }
    else {
        $index = Char::Ekoi8u::index($_[0], $_[1]);
    }
    ...
    if ($index == -1) {
        ...
        return Char::KOI8U::length(CORE::substr $_[0], 0, $index);
    }
}
...
sub Char::KOI8U::rindex($$;$) {
    ...
    my $rindex;
    if (@_ == 3) {
        $rindex = Char::Ekoi8u::rindex($_[0], $_[1], CORE::length(Char::KOI8U::substr($_[0], 0, $_[2])));
    }
    else {
        $rindex = Char::Ekoi8u::rindex($_[0], $_[1]);
    }
    ...
    if ($rindex == -1) {
        ...
        return Char::KOI8U::length(CORE::substr $_[0], 0, $rindex);
    }
}
...
my $qq_substr  = qr{(?: Char::KOI8U::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
sub Char::KOI8U::escape(;$) {
    ...
    while (not /\G \z/oxgc) { # member
        $e_script .= Char::KOI8U::escape_token();
    }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Ekoi8u::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ \s* PREMATCH  \b | \$ \s* \{ \s* PREMATCH  \s* \} | \$ \s* \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Ekoi8u::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Ekoi8u::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Ekoi8u::MATCH()};
    }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Ekoi8u::POSTMATCH()
    elsif (/\G (                 \$ \s* POSTMATCH \b | \$ \s* \{ \s* POSTMATCH \s* \} | \$ \s* \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Ekoi8u::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    # avoid "Error: Runtime exception" of perl version 5.005_03
        ...
        return 'while ($_ = Char::Ekoi8u::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while \s* \( \s* glob \s* \) /oxgc) {
        return 'while ($_ = Char::Ekoi8u::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while \s* \( \s* glob \b /oxgc) {
        return 'while ($_ = Char::Ekoi8u::glob';
    }
...
# subroutines of package Char::Ekoi8u
    elsif (/\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  /oxgc) { $slash = 'm//'; return $1;                  }
    elsif (/\G \b Char::KOI8U::eval       (?= \s* \{ )                /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Char::KOI8U::eval    \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'eval Char::KOI8U::escape'; }
    elsif (/\G \b bytes::substr \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'substr';            }
    elsif (/\G \b chop \b          (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::chop';       }
    elsif (/\G \b bytes::index \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'index';             }
    elsif (/\G \b Char::KOI8U::index \b   (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::KOI8U::index';       }
    elsif (/\G \b index \b         (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::index';      }
    elsif (/\G \b bytes::rindex \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'rindex';            }
    elsif (/\G \b Char::KOI8U::rindex \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::KOI8U::rindex';      }
    elsif (/\G \b rindex \b        (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::rindex';     }
    elsif (/\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::lc';         }
    elsif (/\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::lcfirst';    }
    elsif (/\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::uc';         }
    elsif (/\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::ucfirst';    }
    elsif (/\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::fc';         }
    ...
    elsif (/\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::chr';               }
    elsif (/\G \b bytes::ord    (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::glob';              }
    elsif (/\G \b lc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::lc_';               }
    elsif (/\G \b lcfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::lcfirst_';          }
    elsif (/\G \b uc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::uc_';               }
    elsif (/\G \b ucfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::ucfirst_';          }
    elsif (/\G \b fc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::fc_';               }
    ...
    elsif (/\G \b chr \b           (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! \s* => )                      /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! \s* => )                      /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ekoi8u::glob_';             }
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Ekoi8u::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Char::Ekoi8u::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq       (\#) [ ] (\#) /oxgc) { return 'Char::Ekoi8u::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Ekoi8u::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Ekoi8u::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Ekoi8u::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Ekoi8u::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Ekoi8u::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q        (\#) [ ] (\#) /oxgc) { return 'Char::Ekoi8u::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Ekoi8u::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Ekoi8u::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Ekoi8u::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Ekoi8u::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Ekoi8u::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                ' [ ] '    /oxgc) { return 'Char::Ekoi8u::split' . $e . qq     {' '};  }
        elsif (/\G                " [ ] "    /oxgc) { return 'Char::Ekoi8u::split' . $e . qq     {" "};  }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> @{[Char::Ekoi8u::PREMATCH()]}
            ...
            $e_string .= q{Char::Ekoi8u::PREMATCH()};
            $slash = 'div';
        }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> @{[Char::Ekoi8u::MATCH()]}
        elsif ($string =~ /\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
            $e_string .= q{Char::Ekoi8u::MATCH()};
            $slash = 'div';
        }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> @{[Char::Ekoi8u::POSTMATCH()]}
            ...
            $e_string .= q{Char::Ekoi8u::POSTMATCH()};
            $slash = 'div';
        }
...
# subroutines of package Char::Ekoi8u
        ...
        elsif ($string =~ /\G \b Char::KOI8U::eval       (?= \s* \{ )                /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::KOI8U::eval \b                                /oxgc) { $e_string .= 'eval Char::KOI8U::escape'; $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::substr \b                             /oxgc) { $e_string .= 'substr';            $slash = 'm//'; }
        elsif ($string =~ /\G \b chop \b                                      /oxgc) { $e_string .= 'Char::Ekoi8u::chop';       $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                              /oxgc) { $e_string .= 'index';             $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::KOI8U::index \b                               /oxgc) { $e_string .= 'Char::KOI8U::index';       $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                     /oxgc) { $e_string .= 'Char::Ekoi8u::index';      $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::rindex \b                             /oxgc) { $e_string .= 'rindex';            $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::KOI8U::rindex \b                              /oxgc) { $e_string .= 'Char::KOI8U::rindex';      $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                    /oxgc) { $e_string .= 'Char::Ekoi8u::rindex';     $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ekoi8u::lc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ekoi8u::lcfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ekoi8u::uc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ekoi8u::ucfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ekoi8u::fc';         $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ekoi8u::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ekoi8u::glob';              $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                              /oxgc) { $e_string .= 'Char::Ekoi8u::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                         /oxgc) { $e_string .= 'Char::Ekoi8u::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                              /oxgc) { $e_string .= 'Char::Ekoi8u::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                         /oxgc) { $e_string .= 'Char::Ekoi8u::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                              /oxgc) { $e_string .= 'Char::Ekoi8u::fc_';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                             /oxgc) { $e_string .= 'Char::Ekoi8u::chr_';              $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::ord \b                                      /oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ /\G \b ord \b                                             /oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ /\G \b glob \b                                            /oxgc) { $e_string .= 'Char::Ekoi8u::glob_';             $slash = 'm//'; }
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Ekoi8u::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Char::Ekoi8u::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq       (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Ekoi8u::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Ekoi8u::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Ekoi8u::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Ekoi8u::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Ekoi8u::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Ekoi8u::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q        (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Ekoi8u::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Ekoi8u::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Ekoi8u::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Ekoi8u::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Ekoi8u::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Ekoi8u::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                ' [ ] '    /oxgc) { $e_string .= 'Char::Ekoi8u::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                " [ ] "    /oxgc) { $e_string .= 'Char::Ekoi8u::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
sub character_class {
    ...
    if ($char eq '.') {
        if ($modifier =~ /s/) {
            return '${Char::Ekoi8u::dot_s}';
        }
        else {
            return '${Char::Ekoi8u::dot}';
        }
    }
    else {
        return Char::Ekoi8u::classic_character_class($char);
    }
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Char::Ekoi8u::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Char::Ekoi8u::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Char::Ekoi8u::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Char::Ekoi8u::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Char::Ekoi8u::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Char::Ekoi8u::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Char::Ekoi8u::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Char::Ekoi8u::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Char::Ekoi8u::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Ekoi8u::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Ekoi8u::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Ekoi8u::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Ekoi8u::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Char::Ekoi8u::uc($char[$i]) ne Char::Ekoi8u::fc($char[$i]))) {
            if (CORE::length(Char::Ekoi8u::fc($char[$i])) == 1) {
                $char[$i] = '['   . Char::Ekoi8u::uc($char[$i])       . Char::Ekoi8u::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Char::Ekoi8u::uc($char[$i]) . '|' . Char::Ekoi8u::fc($char[$i]) . ')';
            }
        ...
        # $0 --> $0
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Ekoi8u::ignorecase(' . $char[$i] . ')]}';
            }
        }
        elsif ($char[$i] =~ /\A \$ \{ \s* 0 \s* \} \z/oxms) {
            if ($ignorecase) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Ekoi8u::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Ekoi8u::ignorecase(Char::Ekoi8u::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Ekoi8u::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Ekoi8u::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Ekoi8u::ignorecase(Char::Ekoi8u::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Ekoi8u::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Ekoi8u::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Ekoi8u::ignorecase(Char::Ekoi8u::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Ekoi8u::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $Char::KOI8U::re_t=%s; while($Char::KOI8U::re_t =~ %s){%s local $^W=0; local $Char::KOI8U::re_r=%s; %s$Char::KOI8U::re_t="$`$Char::KOI8U::re_r$'"; pos($Char::KOI8U::re_t)=length "$`$Char::KOI8U::re_r"; } return $Char::KOI8U::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::KOI8U::re_r=CORE::eval $Char::KOI8U::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Char::KOI8U::re_r=%s; %s"%s$Char::KOI8U::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $Char::KOI8U::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::KOI8U::re_r=%s; %s%s="$`$Char::KOI8U::re_r$'"; pos(%s)=length "$`$Char::KOI8U::re_r"; $Char::KOI8U::re_n++} return %s$Char::KOI8U::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Char::KOI8U::re_r=%s; %s%s="%s$Char::KOI8U::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Char::KOI8U::re_r=%s; %s%s="%s$Char::KOI8U::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::KOI8U::re_r=CORE::eval $Char::KOI8U::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_split {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # ${ foo }
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Ekoi8u::ignorecase(' . $1 . ')]}';
            }
        }
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Char::Ekoi8u::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Char::Ekoi8u::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
...
=head1 SYNOPSIS
  ...
  use Char::Ekoi8u;
    ...
    Char::Ekoi8u::lc_;
    Char::Ekoi8u::lcfirst(...);
    Char::Ekoi8u::lcfirst_;
    Char::Ekoi8u::uc(...);
    Char::Ekoi8u::uc_;
    Char::Ekoi8u::ucfirst(...);
    Char::Ekoi8u::ucfirst_;
    Char::Ekoi8u::fc(...);
    Char::Ekoi8u::fc_;
    Char::Ekoi8u::ignorecase(...);
    Char::Ekoi8u::capture(...);
    Char::Ekoi8u::chr(...);
    Char::Ekoi8u::chr_;
    Char::Ekoi8u::glob(...);
  ...
  # "no Char::Ekoi8u;" not supported

=head1 ABSTRACT

This module has run-time routines for use Char::KOI8U software automatically, you
do not have to use.
...
some bugs. Thus, if you happen to find a bug that's in Char::KOI8U software and not
...
This Char::Ekoi8u module first appeared in ActivePerl Build 522 Built under
...
=item Split string
  ...
  @split = Char::Ekoi8u::split(/pattern/,$string,$limit);
  @split = Char::Ekoi8u::split(/pattern/,$string);
  @split = Char::Ekoi8u::split(/pattern/);
  @split = Char::Ekoi8u::split('',$string,$limit);
  @split = Char::Ekoi8u::split('',$string);
  @split = Char::Ekoi8u::split('');
  @split = Char::Ekoi8u::split();
  @split = Char::Ekoi8u::split;
  ...
  at all, Char::Ekoi8u::split returns the original string as a single substring, If it
  ...
  @chars  = Char::Ekoi8u::split(//,  $word);
  @fields = Char::Ekoi8u::split(/:/, $line);
  @words  = Char::Ekoi8u::split(" ", $paragraph);
  @lines  = Char::Ekoi8u::split(/^/, $buffer);
  ...
  print join(":" => Char::Ekoi8u::split(/ */, "hi there"));
  ...
  Char::Ekoi8u::split's pattern is exempt from that wrinkle.)

  The $limit parameter splits only part of a string:

  my ($login, $passwd, $remainder) = Char::Ekoi8u::split(/:/, $_, 3);
  ...
  splits on whitespace just as Char::Ekoi8u::split with no arguments does. Thus,
  Char::Ekoi8u::split(" ") can be used to emulate awk's default behavior. In contrast,
  Char::Ekoi8u::split(/ /) will give you as many null initial fields as there are
  ...
  $string = join(" ", Char::Ekoi8u::split(" ", $string));
  ...
  %head = ("FRONTSTUFF", Char::Ekoi8u::split(/^(\S*?):\s*/m, $header));
  ...
  while (<>) {
      for my $word (Char::Ekoi8u::split()) {
          $count{$word}++;
      }
  }

  The inverse of Char::Ekoi8u::split is join, except that join can only join with the
...
=item Transliteration
  ...
  $tr = Char::Ekoi8u::tr($variable,$bind_operator,$searchlist,$replacementlist,$modifier);
  $tr = Char::Ekoi8u::tr($variable,$bind_operator,$searchlist,$replacementlist);
  ...
  print Char::Ekoi8u::tr('bookkeeper','=~','boep','peob','r'); # prints 'peekkoobor'

=item Chop string

  $chop = Char::Ekoi8u::chop(@list);
  $chop = Char::Ekoi8u::chop();
  $chop = Char::Ekoi8u::chop;

  This subroutine chops off the last character of a string variable and returns the
  character chopped. The Char::Ekoi8u::chop subroutine is used primary to remove the newline
  ...
  since Char::Ekoi8u::chop always shortens the string no matter what's there, and chomp
  is more selective. If no argument is given, the subroutine chops the $_ variable.

  You cannot Char::Ekoi8u::chop a literal, only a variable. If you Char::Ekoi8u::chop a list of
  variables, each string in the list is chopped:

  @lines = `cat myfile`;
  Char::Ekoi8u::chop(@lines);

  You can Char::Ekoi8u::chop anything that is an lvalue, including an assignment:

  Char::Ekoi8u::chop($cwd = `pwd`);
  Char::Ekoi8u::chop($answer = <STDIN>);

  This is different from:

  $answer = Char::Ekoi8u::chop($tmp = <STDIN>); # WRONG

  which puts a newline into $answer because Char::Ekoi8u::chop returns the character
  ...
  In the most general case, Char::Ekoi8u::chop can be expressed using substr:

  $last_code = Char::Ekoi8u::chop($var);
  $last_code = substr($var, -1, 1, ""); # same thing

  Once you understand this equivalence, you can use it to do bigger chops. To
  Char::Ekoi8u::chop more than one character, use substr as an lvalue, assigning a null
  ...
  form of substr, creating something of a quintuple Char::Ekoi8u::chop;
...
=item Index string
  ...
  $byte_pos = Char::Ekoi8u::index($string,$substr,$byte_offset);
  $byte_pos = Char::Ekoi8u::index($string,$substr);
  ...
  $byte_pos = -1;
  while (($byte_pos = Char::Ekoi8u::index($string, $lookfor, $byte_pos)) > -1) {
...
=item Reverse index string
  ...
  $byte_pos = Char::Ekoi8u::rindex($string,$substr,$byte_offset);
  $byte_pos = Char::Ekoi8u::rindex($string,$substr);

  This subroutine works just like Char::Ekoi8u::index except that it returns the byte
  position of the last occurrence of $substring in $string (a reverse Char::Ekoi8u::index).
  ...
  while (($byte_pos = Char::KOI8U::rindex($string, $lookfor, $byte_pos)) >= 0) {
      print "Found at $byte_pos\n";
      $byte_pos--;
  }
...
=item Lower case string
  ...
  You can use the Char::Ekoi8u::fc subroutine for case-insensitive comparisons via Char::KOI8U
  software.

=item Lower case first character of string

  $lcfirst = Char::Ekoi8u::lcfirst($string);
  $lcfirst = Char::Ekoi8u::lcfirst_;
...
=item Upper case string
  ...
  in interpolated strings. For titlecase, use Char::Ekoi8u::ucfirst instead.
...
=item Upper case first character of string
  ...
  $ucfirst = Char::Ekoi8u::ucfirst($string);
  $ucfirst = Char::Ekoi8u::ucfirst_;
  ...
  $titlecase = Char::Ekoi8u::ucfirst(substr($word,0,1)) . Char::Ekoi8u::lc(substr($word,1));
  ...
  $do_not_use = Char::Ekoi8u::ucfirst(Char::Ekoi8u::lc($word));
...
=item Fold case string
  ...
  $fc = Char::Ekoi8u::fc($string);
  $fc = Char::Ekoi8u::fc_;

  New to Char::KOI8U software, this subroutine returns the full Unicode-like casefold of
  ...
  Char::Ekoi8u::fc($a) eq Char::Ekoi8u::fc($b)

  The reliable way to compare string case-insensitively was with the /i pattern
  modifier, because Char::KOI8U software has always used casefolding semantics for
...
=item Make ignore case string
  ...
  @ignorecase = Char::Ekoi8u::ignorecase(@string);

  This subroutine is internal use to m/ /i, s/ / /i, split / /i, and qr/ /i.

=item Make capture number

  $capturenumber = Char::Ekoi8u::capture($string);

  This subroutine is internal use to m/ /, s/ / /, split / /, and qr/ /.

=item Make character

  $chr = Char::Ekoi8u::chr($code);
  $chr = Char::Ekoi8u::chr_;

  This subroutine returns a programmer-visible character, character represented by
  that $code in the character set. For example, Char::Ekoi8u::chr(65) is "A" in either
  ASCII or KOI8-U, not Unicode. For the reverse of Char::Ekoi8u::chr, use Char::KOI8U::ord.

=item Filename expansion (globbing)

  @glob = Char::Ekoi8u::glob($string);
  @glob = Char::Ekoi8u::glob_;
  ...
  The Char::Ekoi8u::glob subroutine grandfathers the use of whitespace to separate multiple
  ...
  @spacies = <"*e f*">;
  @spacies = Char::Ekoi8u::glob('"*e f*"');
  @spacies = Char::Ekoi8u::glob(q("*e f*"));

  If you had to get a variable through, you could do this:

  @spacies = Char::Ekoi8u::glob("'*${var}e f*'");
  @spacies = Char::Ekoi8u::glob(qq("*${var}e f*"));
