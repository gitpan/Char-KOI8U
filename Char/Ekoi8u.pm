...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Char::Ekoi8u::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Char::Ekoi8u::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
        ...
        my $matchsub = sub { Char::Ekoi8u::fc($_[0]) =~ m{\A $pattern \z}xms };
