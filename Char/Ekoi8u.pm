...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Char::Ekoi8u::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Char::Ekoi8u::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  Scans a KOI8-U $string for delimiters that match pattern and splits the KOI8-U
  ...
  pattern, so the delimiters may be of any size and need not be the same KOI8-U
  $string on every match. If the pattern doesn't match at all, Char::Ekoi8u::split returns
  the original KOI8-U $string as a single substring. If it matches once, you get
  ...
  If KOI8-U $string is omitted, the function splits the $_ KOI8-U string.
...
=item Chop string
  ...
  Chops off the last character of a KOI8-U string contained in the variable (or
  KOI8-U strings in each element of a @list) and returns the character chopped.
  The Char::Ekoi8u::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Ekoi8u::index($string,$substr,$position);
  $pos = Char::Ekoi8u::index($string,$substr);

  Returns the position of the first occurrence of $substr in KOI8-U $string.
  The start, if specified, specifies the $position to start looking in the KOI8-U
  $string. Positions are integer numbers based at 0. If the substring is not found,
  the Char::Ekoi8u::index function returns -1.

=item Reverse index string

  $pos = Char::Ekoi8u::rindex($string,$substr,$position);
  $pos = Char::Ekoi8u::rindex($string,$substr);

  Works just like Char::Ekoi8u::index except that it returns the position of the last
  occurence of $substr in KOI8-U $string (a reverse index). The function returns
  -1 if not found. $position, if specified, is the rightmost position that may be
  returned, i.e., how far in the KOI8-U string the function can search.
...
=item Lower case string
  ...
  Returns a lowercase version of KOI8-U string (or $_, if omitted). This is the
...
=item Lower case first character of string
  ...
  Returns a version of KOI8-U string (or $_, if omitted) with the first character
...
=item Upper case string
  ...
  Returns an uppercased version of KOI8-U string (or $_, if string is omitted).
...
=item Make character
  ...
  set. For example, Char::Ekoi8u::chr(65) is "A" in either ASCII or KOI8-U, not Unicode,
  and Char::Ekoi8u::chr(0x82a0) is a KOI8-U HIRAGANA LETTER A. For the reverse of
  Char::Ekoi8u::chr, use Char::KOI8U::ord.
