...
######################################################################
#
# Char::KOI8U - Source code filter to escape KOI8-U script
#
# http://search.cpan.org/dist/Char-Char::KOI8U/
#
...
my $function_ord;         # ord()   to ord()   or Char::KOI8U::ord()
my $function_ord_;        # ord     to ord     or Char::KOI8U::ord_
my $function_reverse;     # reverse to reverse or Char::KOI8U::reverse
my $function_getc;        # getc    to getc    or Char::KOI8U::getc
...
END
    ...
    print Char::KOI8U::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Ekoi8u::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::KOI8U::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Ekoi8u::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::KOI8U::escape_script {
    ...
    # read KOI8-U script
    my $fh = gensym();
    Char::Ekoi8u::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Ekoi8u(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        if (s/^ \s* use \s+ Char::KOI8U \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::KOI8U::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/KOI8U.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/KOI8U.pm $version, but @{[__FILE__]} is version $Char::KOI8U::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Ekoi8u.pm $version, but \\\$Char::Ekoi8u::VERSION is \$Char::Ekoi8u::VERSION" if \$Char::Ekoi8u::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::KOI8U::VERSION) {
                    die "Script $0 required Char/KOI8U.pm $version, but @{[__FILE__]} is only version $Char::KOI8U::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::KOI8U::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::KOI8U::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::KOI8U::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::KOI8U::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::KOI8U::re_t=%s; while($Char::KOI8U::re_t =~ %s){%s local $^W=0; local $Char::KOI8U::re_r=%s; %s$Char::KOI8U::re_t="$`$Char::KOI8U::re_r$'"; pos($Char::KOI8U::re_t)=length "$`$Char::KOI8U::re_r"; } return $Char::KOI8U::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::KOI8U::re_r=eval $Char::KOI8U::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::KOI8U::re_r=%s; %s"%s$Char::KOI8U::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::KOI8U::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::KOI8U::re_r=%s; %s%s="$`$Char::KOI8U::re_r$'"; pos(%s)=length "$`$Char::KOI8U::re_r"; $Char::KOI8U::re_n++} return %s$Char::KOI8U::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::KOI8U::re_r=%s; %s%s="%s$Char::KOI8U::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::KOI8U::re_r=%s; %s%s="%s$Char::KOI8U::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::KOI8U::re_r=eval $Char::KOI8U::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/KOI8U.pm and Char/Ekoi8u.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::KOI8U;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::KOI8U;
  use Char::KOI8U ver.sion;             --- require minimum version
  use Char::KOI8U ver.sion.0;           --- expects version (match or die)

  # "no Char::KOI8U;" not supported

  or

  $ perl Char/KOI8U.pm KOI8-U_script.pl > Escaped_script.pl.e
...
Char::KOI8U software is "middleware" between perl interpreter and your Perl script
written in KOI8-U.
...
By "use Char::KOI8U;", it automatically interpret your script as KOI8-U. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/KOI8U.pm, Char/Ekoi8u.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Ekoi8u.pm
works well.
...
=head1 Software Composition
   ...
   Char/KOI8U.pm               --- source code filter to escape KOI8-U
   Char/Ekoi8u.pm              --- run-time routines for Char/KOI8U.pm
...
You need write 'use Char::KOI8U;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::KOI8U;
  ---------------------------------

=head1 Calling 'Char::Ekoi8u::ignorecase()' (Char/KOI8U.pm provides)

Char/KOI8U.pm applies calling 'Char::Ekoi8u::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Ekoi8u::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Ekoi8u.pm provides)
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/KOI8U.pm provides)

Char/KOI8U.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/KOI8U.pm and Char/Ekoi8u.pm provide)

Insert 'Char::Ekoi8u::' at head of function name. Char/Ekoi8u.pm provides your script Char::Ekoi8u::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Char::Ekoi8u::lc        Character
  lcfirst     Char::Ekoi8u::lcfirst   Character
  uc          Char::Ekoi8u::uc        Character
  ucfirst     Char::Ekoi8u::ucfirst   Character
  fc          Char::Ekoi8u::fc        Character
  chr         Char::Ekoi8u::chr       Character
  glob        Char::Ekoi8u::glob      Character
  -------------------------------------------
...
=head1 Un-Escaping bytes::* Subroutines (Char/KOI8U.pm provides)

Char/KOI8U.pm removes 'bytes::' at head of subroutine name.
...
'use utf8;' to 'use Char::KOI8U;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Ekoi8u.pm provides these
  functions.
...
Char::KOI8U software remains one language and one interpreter.
