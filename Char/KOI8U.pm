...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Ekoi8u::uc($char[$i]) ne Char::Ekoi8u::lc($char[$i]))) {
            $char[$i] = '[' . Char::Ekoi8u::uc($char[$i]) . Char::Ekoi8u::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Char::KOI8U/Char::KOI8U/JA.pm
